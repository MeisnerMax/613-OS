...
assert(runtimeVerifier.includes("development?.projects === 5"), "Portfolio verifier must enforce the five-project staged Development scope.");
assert(runtimeVerifier.includes("development.packages === 360"), "Portfolio verifier must enforce the 360-package staged Development scope.");
...
assert(runtimeVerifier.includes("oldPost.packages === 72"), "Portfolio verifier must enforce 72 Old Post packages.");
assert(runtimeVerifier.includes('oldPost.assetId === "A012"'), "Old Post Asset link verification is missing.");
assert(runtimeVerifier.includes("oldPost.done === 2"), "Old Post completed-package baseline is missing.");
assert(runtimeVerifier.includes("oldPost.inProgress === 5"), "Old Post in-progress baseline is missing.");
assert(runtimeVerifier.includes("oldPost.notStarted === 65"), "Old Post not-started baseline is missing.");
...
assert(runtimeVerifier.includes("legacy-development-oldpost-import-2026-08-08"), "Old Post migration marker verification is missing.");
