...
import { diagnoseGoogleSheetsReadOnly } from "@/lib/verification/google-sheets-diagnostics";
...
export async function GET() {
  ...
  try {
    const diagnostic = await diagnoseGoogleSheetsReadOnly(session.accessToken);
    if (!diagnostic.ok) {
      return NextResponse.json(
        {
          ok: false,
          ...sessionInfo,
          diagnostic,
          error: "Google Sheets read access is blocked before parity can run.",
        },
